...
dma_chan3 = 3
testsrc3 = array.array('i', [0, 0, 0])
testsrc3_ptr = array.array('i', [addressof(testsrc3)])
testdst3 = array.array('i', [0, 0, 0, 0, 1])
ctrl = (DMA_IRQ_QUIET |
        DMA_TREQ(DREQ_PIO0_RX1) |
        DMA_WR_INC |
        #DMA_RD_INC |
        DMA_DATA_WIDTH(data_32))

IOreg_write(DMA_RD_ADDR(dma_chan3), PIO0_RXF1_ADDR)
IOreg_write(DMA_WR_ADDR(dma_chan3), addressof(testdst3))
IOreg_write(DMA_TR_COUNT_ADDR(dma_chan3), len(testdst3))
...
dma_chan2 = 2
testsrc2 = array.array('i', [2]*(len(testdst3)-1))
testsrc2_ptr = array.array('i', [addressof(testsrc2)])
testdst2 = array.array('i', [0, 0, 0])
ctrl = (DMA_IRQ_QUIET |
...
IOreg_write(DMA_RD_ADDR(dma_chan2), addressof(testsrc2))
IOreg_write(DMA_WR_ADDR(dma_chan2), PIO0_TXF1_ADDR)#addressof(testdst2))
IOreg_write(DMA_TR_COUNT_ADDR(dma_chan2), len(testsrc2))
IOreg_write(DMA_CTRL_ADDR(dma_chan2),  ctrl)
machine.mem32[DMA_CTRL_ADDR(dma_chan2)] |= DMA_EN
...
IOreg_write(DMA_RD_ADDR(dma_chan1), addressof(testsrc1))
IOreg_write(DMA_WR_ADDR(dma_chan1), addressof(testdst1))
IOreg_write(DMA_TR_COUNT_ADDR(dma_chan1), 1)
IOreg_write(DMA_CTRL_ADDR(dma_chan1),  ctrl)
machine.mem32[DMA_CTRL_ADDR(dma_chan1)] |= DMA_EN
...
sm0 = rp2.StateMachine(0, prog=PIOSignal)
sm0.active(1)
sm1 = rp2.StateMachine(1, prog=PIOSignal)
sm1.active(1)
